...
class SDLPlatform : Platform {
    ...
    override void closeWindow(Window w) {
    	Log.d("calling closeWindow on platform");
    ...
    override int enterMessageLoop() {
        ...
        //calculate average FPS
       	uint frameCount=0;
        ulong fpsTimer = SDL_GetTicks();
        float avgFPS;
        
        //to cap the loop rate
        ulong lastFrame = SDL_GetTicks();
        const uint targetFrameRate = 300; //this allow a very responsive GUI
        const uint targetTime = 1000 / targetFrameRate;
        while(!quit) {
        
            //redrawWindows();
           /* ++frameCount;
        	if(frameCount == 30) {
        		avgFPS = frameCount / (( SDL_GetTicks() - fpsTimer) / 1000.0);
            	fpsTimer = SDL_GetTicks();
            	frameCount = 0;
            	//Log.i("avg FPS is " ~ to!string(avgFPS));
            }*/
            
            if(SDL_GetTicks() - lastFrame < targetTime) SDL_Delay(targetTime -to!uint( SDL_GetTicks() - lastFrame));//so we achive roughly targetFrameRate
            lastFrame = SDL_GetTicks();
            
            if(onLoop !is null) onLoop();
            
            
            if (SDL_PollEvent(&event)) { //this instead of WaitEvent make the loop keep going
